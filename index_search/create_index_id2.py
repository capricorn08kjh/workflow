...
def get_payload_root(obj, elasticdump=False, payload_path=None):
    """
    - elasticdump=True: {"index":...}/{\"create\":...} 단독 라인은 None (skip)
    - payload_path 지정 시: 해당 dotted 경로
    - elasticdump=True & payload_path 미지정: 기본 '_source'
    - 일반 NDJSON/JSONL: obj 자체
    """
    if elasticdump and isinstance(obj, dict) and len(obj) == 1 and any(k in obj for k in META_ACTION_KEYS):
        return None  # 메타라인 스킵
    if payload_path:
        base = obj
    ...
    try:
        for file_id, p in enumerate(files):
            with open(p, "rb") as f:
                offset = 0
                line = f.readline()
                pending_id = None  # 메타라인의 _id (elasticdump 2-라인일 때)
                while line:
                    obj = None
                    try:
                        obj = json.loads(line)
                    ...
                    # 메타라인: {"index":...} / {"create":...}
                    if elasticdump and isinstance(obj, dict) and len(obj) == 1 and any(k in obj for k in META_ACTION_KEYS):
                        meta = obj.get("index") or obj.get("create")
                        pending_id = (meta or {}).get("_id")
                        offset += len(line)
                        line = f.readline()
                        continue
                    ...
                    if base is not None:
                        ...
                        if want_root_id:
                            ...
                            if root_id_val is not None:
                                txn.put(make_key_bytes("_id", str(root_id_val)), v, db=kv_db, dupdata=True)
                                total_put += 1
                                if commit_interval > 0 and (total_put % commit_interval == 0):
                                    txn.commit(); txn = env.begin(db=kv_db, write=True)
...
def main():
    ...
    ap.add_argument("--keys", nargs="+", required=True, help="예: _source _id (둘 다 인덱싱)")
    ...
    ap.add_argument("--ngram", action="append", default=[], help="예: --ngram \"_source.title:2\"")
    args = ap.parse_args()
